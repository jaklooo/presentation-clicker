...
export default function HostPage({ params }) {
    ...
    function onFileChange(event) {
        ...
        if (nextFile) {
            if (nextFile.name.toLowerCase().endsWith('.pptx') || nextFile.name.toLowerCase().endsWith('.ppt')) {
            ...
            {showPptxWarning && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-gray-800 rounded-2xl p-8 max-w-md w-full border border-gray-700 shadow-2xl">
                        ...
                        <h3 className="text-xl font-bold text-center mb-2">PowerPoint File Detected</h3>
                        <p className="text-gray-400 text-center mb-6">
                            Browsers cannot natively display PowerPoint files. For the best experience and perfect rendering:
                        </p>
                        <div className="bg-gray-900/50 rounded-lg p-4 mb-6 border border-gray-700">
                            <p className="text-sm text-gray-300 text-center">
                                Please save your presentation as <strong>PDF</strong>
                                <br />
                                <span className="text-xs text-gray-500">(File &rarr; Save As &rarr; PDF)</span>
                            </p>
                        </div>
                        <button
                            onClick={() => setShowPptxWarning(false)}
                            className="w-full py-3 bg-blue-600 hover:bg-blue-500 rounded-xl font-semibold transition-colors"
                        >
                            Got it, I'll upload a PDF
                        </button>
                    </div>
                </div>
            )}
